...
export default function(stream) {
	...
	// Support abbreviations that end with +
	// Like select+, ul+, ol+ etc.
	if (stream.peek() == OP_SIBLING) {
		let currentPos = stream.pos;
		stream.next();
		if (stream.eof() || stream.peek() == GROUP_END || stream.peek() == ASTERISK) {
			node.name += String.fromCharCode(OP_SIBLING);
